...
void *removeFirst(linkedList *list) /* Returns a pointer to the data of the removed node */
{
    ...
    if(list->head != NULL) /* Check if the list is not empty */
    {
...
{
    Player *player = (Player *)data; /* Casting the void data to a type of Player */
    free(player);
}

void freeSnake(void *data) /* Frees the memory allocated for the data of the linked list */
{
    Snake *snake = (Snake *)data; /* Casting the void data to a type of Snake */
    free(snake);
}